...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for d in item_list:
		...
		row.update({
			...
			'stock_qty': d.qty, # change
			'stock_uom': d.uom # change
		})
...
def get_conditions(filters):
	...
	# custom change
	if filters.get("item_group"):
		item_group_details = frappe.db.get_value("Item Group", filters.get("item_group"), ["lft", "rgt"], as_dict=1)
		conditions +=  """and ifnull(`tabSales Invoice Item`.item_group, '')  in (select ig.name from `tabItem Group` ig \
